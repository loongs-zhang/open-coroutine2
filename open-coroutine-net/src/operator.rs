...
cfg_if::cfg_if! {
    if #[cfg(all(target_os = "linux", feature = "io_uring"))] {
        ...
        use open_coroutine_core::version::{current_kernel_version, kernel_version};
...
impl Default for Operator<'_> {
    fn default() -> Self {
        Self::new(1).expect("create operator failed")
    }
}
...
impl Operator<'_> {
    ...
    fn token() -> u64 {
        if let Some(co) = SchedulableCoroutine::current() {
            ...
            cstr.as_ptr().cast::<c_void>() as u64
        } else {
            unsafe {
                cfg_if::cfg_if! {
                    if #[cfg(windows)] {
                        windows_sys::Win32::System::Threading::GetCurrentThread() as u64
                    } else {
    ...
    pub fn fsync(&self, fd: c_int) -> std::io::Result<c_int> {
        impl_if_support!(*SUPPORT_FSYNC, {
            let user_data = Self::token();
            let entry = Fsync::new(Fd(fd)).build().user_data(user_data);
            self.push_sq(entry)?;
            todo!()
        })
    }
    ...
    pub fn shutdown(&self, socket: c_int, how: c_int) -> std::io::Result<c_int> {
        impl_if_support!(*SUPPORT_SHUTDOWN, {
            let user_data = Self::token();
            let entry = Shutdown::new(Fd(socket), how).build().user_data(user_data);
            self.push_sq(entry)?;
            todo!()
        })
    }
    ...
    pub fn close(&self, fd: c_int) -> std::io::Result<c_int> {
        impl_if_support!(*SUPPORT_CLOSE, {
            let user_data = Self::token();
            let entry = Close::new(Fd(fd)).build().user_data(user_data);
            self.push_sq(entry)?;
            todo!()
        })
    }
        ...
        impl_if_support!(*SUPPORT_RECV, {
            ...
            if let Some(coroutine) = SchedulableCoroutine::current() {
                if let Some(suspender) = SchedulableSuspender::current() {
                    // 此时协程已经是syscall状态了，详见LinuxNetSyscallFacade
                    suspender.suspend();
                    //回来的时候系统调用已经执行完了
                    match coroutine.state() {
                        CoroutineState::SystemCall(_, Syscall::recv, state) => {
                            if SyscallState::Computing == state {
                                coroutine.syscall((), Syscall::recv, SyscallState::Finished)?;
                            } else {
            ...
            // assert!(
            //     self.wait_table.insert(token, arc.clone()).is_none(),
